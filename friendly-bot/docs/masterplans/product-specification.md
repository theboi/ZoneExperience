...
| Scope | Friendly Bot local MVP for Telegram NBNC engagement, event flows, operational users, and human connection |
...
Operational profiles store login identity, interests, `cg_name`, contact URL, availability, capacity, and event attendance. NBNCs do not provide DOBs. Prefilled operational accounts authenticate using normalized name and DOB.
...
Outside an active event, an NBNC can request directions to Star and information about NCC. During an event, the event's approved checkpoint and timestamp flows govern the available experience.
...
The bot asks for one interest and whether the NBNC wants to join the human or have the human join them. It ranks only eligible, attending operational users with available capacity. The chosen human is assigned without accept or decline, notified with limited NBNC context, and their Telegram URL is shared with the NBNC.
...
Button IDs are stable and namespaced. Non-event reusable buttons may be used later. Event buttons stop at the event interaction boundary and respond that the event is over.

The bot uses warm, direct onboarding language and calm, actionable error language. It never exposes internal errors, secrets, DOBs, or Telegram IDs. When routing context is ambiguous, it asks: “Sorry, which message were you referring to?” When no configured flow matches, it says: “Sorry, I didn't understand your request.”
...
System-global and event-global roots are automatic checkpoints. Every timestamp has one automatic root flow. Empty action lists may be used to expose children without sending or processing anything.
...
Events define highkey behavior, door times, an interaction boundary, an event checkpoint, a latecomer flow, timestamp roots, attendees, and delivery audiences.

One highkey event before door close enrolls automatically. Multiple events with any highkey require a forced event choice. Lowkey-only events require confirmation. After door close but before interaction end, confirmation still enrolls the NBNC and runs the latecomer flow. One overlapping event is active per NBNC.

Timestamp audiences are all NBNCs, all servers, all leaders, event NBNCs, event servers, event leaders, or all event attendees. Role inheritance applies to operational audiences.
...
PostgreSQL stores users, operational profiles and logins, event configuration and attendance, immutable flow versions, open flow selections, conversations and persona cursors, human matches and capacity, Telegram update state, timestamp deliveries, outbound delivery attempts, and sanitized diagnostics.
...
Secrets are external to source control and redacted from logs and notifications. Database constraints protect login ownership, event activity, matching capacity, flow keys, and idempotency. Conversation and operational data require application authorization and restricted local database access.

Every emitted application error and debug diagnostic creates a sanitized notification for every admin and a structured diagnostic record. Action failures stop later actions, retry safe failures, and run configured fixed fallbacks after exhaustion.

## 7. Acceptance requirements

- New and existing NBNC onboarding follows the approved name and event rules.
...
- Event enrollment, overlaps, latecomers, timestamps, audiences, and expiry behave as specified.
