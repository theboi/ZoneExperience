...
- Reconcile before Task 1: F01 planning commit `9b5b4e5c7fef5b3a215572cc3f74174590dc5a2d` makes `users.role` the sole role source, removes `operational_profiles.role`, and publishes frozen UoW DTO/repository contracts; matching filters joined user role. F01 still must include the canonical `send_service_choice_buttons` action discriminator. R03 must expose selected routing items as `RoutingDecision`/selected keys, never a `FlowSelection` wrapper. Stop for an upstream repair if either remaining item is absent; do not adapt around it.
