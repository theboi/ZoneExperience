...
| `DeliveryRepository` | `claim_timestamp_delivery(service_timestamp_id: UUID, user_id: UUID) -> bool`; `enqueue(delivery: NewOutboundDelivery) -> OutboundDeliveryRecord`; `claim_next_safe(*, now: datetime, lease_duration: timedelta = timedelta(minutes=1)) -> OutboundDeliveryRecord | None`; `renew_claim(delivery_id: UUID, *, claim_token: UUID, now: datetime, lease_duration: timedelta = timedelta(minutes=1)) -> OutboundDeliveryRecord`; `start_attempt(delivery_id: UUID, correlation_id: UUID, *, claim_token: UUID, started_at: datetime) -> DeliveryAttemptRecord`; `finish_attempt(delivery_id: UUID, attempt_id: UUID, outcome: DeliveryOutcome, *, now: datetime, retry_at: datetime | None = None, safe_error: str | None = None, confirmed_telegram_message_id: int | None = None) -> None` |
...
## 2026-09-13 delivery-contract repair

Whole-F01 review established that T02 could not safely implement its outbox against the original delivery DTO and repository surface: claims did not carry a send message or lease token, retries reused `created_at`, and attempts discarded correlation IDs. The repair is an additive `0002_delivery_contract` migration. It adds `eligible_at`, recoverable-unstarted claim token/expiry, confirmed Telegram message ID, and attempt correlation ID, plus only the due/expired-claim partial indexes used by the atomic claim predicate. `OutboundDeliveryRecord.message` is a frozen provider-neutral message; a caller must commit a successful token-fenced `start_attempt` before sending. No T02/R03/I04 worker artifact or product/living authority changes because the repair is confined to F01's existing durable-delivery ownership boundary.
