...
| Date | 2026-09-12 |
| Owner | I04 |
| Authority inputs | Product specification, architecture masterplan (`ARCH-001` through `ARCH-013`), approved MVP design, canonical Zone X service document, frozen implementation masterplan, reconciled F01 contract at `9b5b4e5c7fef5b3a215572cc3f74174590dc5a2d`, reconciled T02 contract at `23350575bfaff92de934d2b47cdb89c498c0ad0d`, and reconciled R03 contract at `50ab700c2256c9c046b481389f6908b9ea3550c4` on `origin/main` |
...
| R03 | `ConstrainedRouter`, `CandidateAssembler`, `OpenRouterGateway`, `PersonaMaintenanceService`, `MatchingService`, prompt-safe `IncomingText`, `RoutingTerminal`, and a `RoutingResult` whose selected candidate keys are named `RoutingDecision`/`selected_keys`, never `FlowSelection`. I04 supplies no structured Telegram ID, DOB, contact URL, UUID, raw profile record, event payload, or diagnostic to an R03 prompt DTO. |

Coordinator normalization is binding at execution: `users.role` is the one role source. `operational_profiles` has no duplicate role column; matching joins its profile attributes to `users.role` and filters that user role exactly. I04 fixture setup and assertions write/read roles through users only. This corrects the F01 plan's duplicate-role schema description and requires the G1 implementation and the T02/R03 consuming queries to be reconciled before I04 starts.

The canonical Zone X document also uses `send_service_choice_buttons`, while F01's listed action union omits it. G1 must register and validate that discriminator, including its button/template data, before the seed can publish. The action's executor is I04-owned. R03's dated text calls selected routing items `FlowSelection`; that rejected wrapper name must be normalized to `RoutingDecision` or a selected key at G2. These are explicit prerequisite reconciliations, not invitations for I04 to add an adapter.
