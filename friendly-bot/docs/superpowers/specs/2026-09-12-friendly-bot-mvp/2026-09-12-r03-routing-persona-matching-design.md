...
`hyperparameters.py` owns the safe defaults: `OPENROUTER_MODEL = "mistralai/mistral-nemo"`, ZDR enforcement, `OPENROUTER_TIMEOUT_SECONDS`, `OPENROUTER_MAX_RESPONSE_BYTES = 65,536`, `ROUTING_MAX_ATTEMPTS`, `PERSONA_IDLE_AFTER = timedelta(hours=48)`, and `PERSONA_MAX_UNSUMMARIZED_TOKENS`. `OpenRouterSettings` reads `OPENROUTER_API_KEY`, the optional non-secret `OPENROUTER_MODEL` and `FRIENDLY_BOT_OPENROUTER_ENFORCE_ZDR` overrides, plus R03's non-secret `FRIENDLY_BOT_OPENROUTER_INPUT_OUTPUT_LOGGING_ATTESTATION` from the environment. ZDR defaults to `true`; an operator may set it to `false` only for an explicitly approved compatibility test, while `data_collection: "deny"` remains request-enforced. The logging setting defaults to `false` (unattested) and only accepts `disabled-globally-or-friendly-bot-key-excluded` to enable a gateway. It is an operator attestation that the account has disabled OpenRouter Input & Output Logging globally or explicitly excluded the dedicated Friendly Bot key; it is not proof of either account state and is deliberately usable by I04 through `OpenRouterGateway.from_environment()` without exposing a Telegram ID, DOB, or credential.

`OpenRouterGateway` refuses construction without that exact attestation. It posts to `https://openrouter.ai/api/v1/chat/completions` with the configured model and `provider={"zdr": true, "data_collection": "deny"}` by default; a deliberate non-ZDR test omits only `zdr`, never `data_collection: "deny"`. It never opts into `logprobs`, debug, trace, or metadata logging fields: `logprobs` controls returned token probabilities, not OpenRouter prompt storage, so it is never described or used as a logging control. A response is accepted only when its entire parsed assistant content is a single JSON object `{"key":"<one allowed key>"}`. Any unavailable compliant endpoint, non-2xx response after bounded retry, malformed object, unknown key, or multiple key is a closed failure with no raw request or response attached.

The decoded-result lifecycle is the privacy boundary. `GatewayHttpClient.post(..., decoder=...)` accepts the operation-specific decoder and returns only a validated configured key, a nonempty persona summary, a validated local alias, or a typed static protocol/transport failure; it never returns a raw provider response object to a public gateway operation. For a stdlib non-2xx result or `HTTPError`, it returns a status-only transport failure and never reads the provider body. For a 2xx result, it reads at most `OPENROUTER_MAX_RESPONSE_BYTES + 1` bytes, rejects an oversize body, parses and validates in the same low-level scope, and clears the local byte buffer before returning. Ordinary `Exception` failures while reading, parsing, validating, or closing—such as `IncompleteRead`, malformed/deep JSON recursion, and discard/close failures—become safe protocol or retryable transport outcomes only after that scope exits. The lifecycle deliberately does not catch `BaseException` or cancellation. `_post` clears its outbound headers and prompt payload before returning a final safe outcome; the stdlib worker receives an isolated request snapshot and clears it after its own lifecycle, so caller cleanup never makes a cancelled worker dispatch an empty payload. Public methods construct `GatewayError` only from the safe outcome.

The gateway emits no logs. Public error messages, arguments, causes, contexts, and stored error attributes contain only typed safe summaries, never raw provider request/response bodies, the API key, Telegram identifiers, DOB, or user text. Regression tests traverse gateway traceback-frame locals for provider sentinels after actual stdlib error paths. This boundary is intentionally not a universal claim about arbitrary live Python frame inspection: normal execution necessarily has transient `self`, request, prompt, and credential references while it runs. It guarantees that raw provider response bytes do not survive in a returned response object or in a raised gateway error trace after the decoded lifecycle has completed.
...
Focused R03 evidence is: gateway payload/privacy and fail-closed tests, including bounded stdlib reads, malformed/deep bodies, `HTTPError` status-only handling, `IncompleteRead`, discard/close failure, oversize rejection, cancellation propagation, outbound request cleanup, and traceback-local negative assertions across selection, persona, and matching; current/global/reusable/native-reply/multi-key/ambiguity/no-match routing tests; idle/token/cursor persona tests; normal role/attendance/capacity/exclusion/rematch tests; safety leader/staff/always-available/no-fallback tests; and concurrent reservation integration tests. `ruff check .`, `ruff format --check .`, and `mypy src/friendly_bot` run after R03 focused suites. The source repair is committed and awaits re-review; it is not source approval. User-facing and product scope are unchanged. R03 implementation cannot become operational until an account operator supplies a redacted receipt showing OpenRouter Input & Output Logging is disabled globally or the dedicated Friendly Bot API key is excluded; the local environment attestation is not that receipt and G2 remains not passed.
