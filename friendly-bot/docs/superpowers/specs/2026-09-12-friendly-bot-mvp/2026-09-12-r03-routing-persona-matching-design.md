...
`OpenRouterGateway` refuses construction without that exact attestation. It posts to `https://openrouter.ai/api/v1/chat/completions` with the configured model and `provider={"zdr": true, "data_collection": "deny"}`. It never opts into `logprobs`, debug, trace, or metadata logging fields: `logprobs` controls returned token probabilities, not OpenRouter prompt storage, so it is never described or used as a logging control. A response is accepted only when its entire parsed assistant content is a single JSON object `{"key":"<one allowed key>"}`. Any unavailable compliant endpoint, non-2xx response after bounded retry, malformed object, unknown key, or multiple key is a closed failure with no raw request or response attached. The centralized transport boundary treats ordinary provider/client exceptions, including an interrupted stdlib `response.read()` whose `http.client.IncompleteRead.partial` carries provider bytes, as bounded retry failures; it does not catch `BaseException` or cancellation. It records that failure only in a local retry flag and raises the final typed transport error after leaving the exception scope. For response-lifecycle privacy, the typed response contract consumes and clears successful stdlib bytes when JSON is decoded, requires fakes to discard after parsing, and makes `HTTPError` responses status-only without reading their bodies. JSON-decoding exceptions are normalized to invalid protocol values and structural parsing returns a value or `None`; callers raise typed protocol errors only after raw-bearing parser frames and locals are gone. Thus malformed transport, envelope, or content bodies are not retained by `__cause__`, `__context__`, error message text, error arguments, stored exception attributes, or gateway traceback-frame locals.
