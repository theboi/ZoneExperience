...
`config.settings.DatabaseSettings` reads only `FRIENDLY_BOT_DATABASE_URL` and defaults to `postgresql+asyncpg://friendly_bot:friendly_bot@127.0.0.1:5832/friendly_bot`. Secrets remain environment-supplied and are never printed. `DatabaseSettings.redacted_url()` replaces the password before an operational result is logged.
...
| --- | --- |
| Identities | `users` (unique nullable `telegram_user_id`, display name, role, `is_admin`); `operational_profiles` (one-to-one user, normalized name/DOB, exact role, contact URL, `always_available`, `capacity >= 0`); `operational_logins` (unique profile and user attachment history) |
...
| `operational_profiles` | `id`, `user_id`, `normalized_name`, `dob`, `role`, `interests` JSONB, `cg_name?`, `telegram_contact_url?`, `always_available`, `capacity`, `reserved_capacity`, `created_at`, `updated_at` |
...
The UoW also exposes `UserRepository`, `OperationalProfileRepository`, `ServiceRepository`, `AttendanceRepository`, `ConversationRepository`, `PersonaRepository`, `MatchRepository`, and `DiagnosticRepository`, with direct methods for their tables' externally required operations. F01 defines these contracts and persistence behavior; T02/R03/I04 consume them rather than creating side tables or independent sessions.
