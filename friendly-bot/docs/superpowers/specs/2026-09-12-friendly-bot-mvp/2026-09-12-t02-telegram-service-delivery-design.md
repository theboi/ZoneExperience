...
| Date | 2026-09-12 |
| Owner | T02 |
| Authority inputs | Product specification, architecture masterplan, approved MVP design, Zone X fixture, implementation masterplan, EdenMind transport reference, and F01 planning handoff at `origin/main` `7fb3fd826bc3f06fef2de34a9daeec75260ccc8a` |
...
| `UpdateRepository.claim_update(telegram_update_id, received_at) -> bool` and singleton `telegram_poll_state.next_update_offset` | Claim duplicate protection before business processing; advance durable offset only in the same successful commit. |
| `DeliveryRepository.claim_timestamp_delivery(service_timestamp_id, user_id) -> bool` and `.enqueue(NewOutboundDelivery)` | Exactly-once logical timestamp recipient delivery and durable transport work creation. |
| `OpenSelectionRepository.expire_service_bound(service_id, at)` | Expire Zone X-bound selections at `interaction_ends_at`; never infer expiry from memory. |
...
| `FlowVersionRepository`, `ServiceRepository`, `AttendanceRepository`, `UserRepository`, `OperationalProfileRepository`, `ConversationRepository`, and `DiagnosticRepository` | Typed reads/writes for service lifecycle, exclusive Telegram attachment, messages, and sanitized failures. |

F01 leaves detailed method signatures on the general repositories to its implementation. Before T02 execution, the worker must inspect the **implemented** G1 exports and add only an additive request through the coordinator if a required typed method is absent; it must not use ORM internals or a local compatibility shim. The resulting T02 package exports the DTOs below for I04 composition.
...
Every user-facing send is first a `NewOutboundDelivery` with an operation-derived idempotency key and immutable JSON payload. An outbox worker claims one pending delivery transactionally, appends an attempt with its correlation ID, and then crosses the Telegram network boundary exactly once for that claim.
