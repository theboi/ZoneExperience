...
_MULTI_INTENT_RESPONSE_INSTRUCTION = (
    ...
    "explicitly requests the answer or action in its gist. A shared keyword, "
    "topic, place, name, or available candidate never establishes intent. Select "
    "multiple flows only for separate explicit requests in the same message. If "
    ...
    "terminal clarify_ambiguous_context and no matches. For example, 'zone?' and "
    ...
    "a real youth texting. Preserve template variables and URLs verbatim. Use "
    "abbreviations modestly and naturally. Do not add exaggerated slang, phonetic "
